...
// rolling update. An env change is used rather than an image change on purpose: it is still a
// template diff that is not replicas-only, so it forces the recreate path, but it needs no
// second pullable image. That keeps the case runnable on clusters without registry access.
const (
		...
		"rbgset controller", func() {
					...
					gomega.Eventually(
						func() bool {
							...
							for i := range children {
								if !hasRolloutMarker(children[i].Spec.Roles[0]) {
									return false
								}
							}
							rolledUIDs = framework.RbgSetV2ChildUIDs(children)
							return true
						}, utils.Timeout, utils.Interval,
					).Should(gomega.BeTrue())

					// Both groups were recreated: the deterministic names are kept, the UIDs are not.
					gomega.Expect(rolledUIDs).Should(gomega.HaveLen(2))
					for name, uid := range initialUIDs {
						gomega.Expect(rolledUIDs[name]).NotTo(
							gomega.Equal(uid), "group %s should have been deleted and recreated", name,
						)
					}
			...
			ginkgo.It(
				"surge group survives until the rollout completes", func() {
					...
					// maxUnavailable is 1 of 2, so one group keeps serving on its own and the surge
					// group is extra capacity on top. That capacity is what lets the rollout spend
					// its budget, so withdrawing it before the last base group is serving would drop
					// the set below what the user paid for.
					var rolledUIDs map[string]types.UID
					sawSurge := false
					gomega.Eventually(
						func() bool {
