...
func (r *RoleBasedGroupSetReconciler) reconcileRolling(
	...
	var scaleOnly []*workloadsv1alpha2.RoleBasedGroup
	var metadataDrift []*workloadsv1alpha2.RoleBasedGroup
	...
	complete := r.rolloutComplete(rbgset, children, replicas)
	...
	if !params.paused && inProgress && len(keptSurge) < params.maxSurge {
		...
		for ordinal := replicas; ordinal < replicas+params.maxSurge; ordinal++ {
			...
			rbg := newRBGForSet(rbgset, ordinal)
			surgeToCreate = append(surgeToCreate, rbg)
		}
...
// rolloutComplete reports whether every base ordinal exists on the current template and is
// serving, which is the point where surge capacity has nothing left to cover. Readiness is
// part of the test because a group that was just recreated matches the template at once but
// is not serving yet, and withdrawing the surge groups at that moment would leave fewer than
// replicas-maxUnavailable groups ready for as long as the new one takes to come up.
func (r *RoleBasedGroupSetReconciler) rolloutComplete(
	rbgset *workloadsv1alpha2.RoleBasedGroupSet, children classifiedChildren, replicas int,
) bool {
	...
	serving := 0
	for _, rbg := range children.base {
		if !r.isServing(rbg) || !r.rolesEqual(rbg.Spec.Roles, template) {
			return false
		}
		serving++
	}
	return serving == replicas
}
